...
grep -Fq '`prompt/`を読まない' "$TDD_SKILL" && grep -Fq '[設計書モード](FROM_DOC.md)' "$TDD_SKILL" && grep -Fq '`$tdd --from-doc`だけが読む' "$TDD_FROM_DOC" && grep -Fq '参照先がない、または未完了項目がなければ変更せず報告' "$TDD_FROM_DOC" && ok "tddは通常依頼と明示的な設計書modeを分離" || ng "tddの入力・完了処理の分岐が不正"
...
grep -Fq '## 調査' "$TDD_SKILL" && grep -Fq '`path:line`' "$TDD_SKILL" && grep -Fq '必須事実が足りなければ追加調査' "$TDD_SKILL" && ! grep -Fq 'SUBAGENT_RULES.md' "$TDD_SKILL" && ok "tddフローはメインの調査と限定的な委任を共有" || ng "tddフローの直接調査境界が不正"
...
grep -Fq '選択済みtest、直接の回帰test、変更packageのtypecheck' "$TDD_SKILL" && grep -Fq '無関係なpackage・repository全体へ広げず' "$TDD_SKILL" && grep -Fq 'Prisma `format`・`validate`・`generate`' "$TDD_SKILL" && ok "tddフローは調査commandと最終検証の範囲を固定" || ng "tddフローの調査commandまたは最終検証が曖昧"
...
grep -Fq '../polish/PROCEDURE.md' "$TDD_FROM_DOC" && grep -Fq '実変更pathをまとめて[polish]' "$TDD_FROM_DOC" && grep -Fq 'bash [skills_root]/tdd/mark-prompt-done.sh <機能名>' "$TDD_FROM_DOC" && ok "tdd は実変更pathのpolish後だけindexを更新" || ng "tdd のpolish対象または品質ゲートが不正"
