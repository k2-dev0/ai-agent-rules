...
grep -Fq 'This is an implementation action, not an investigation, review, approval, or scenario-classification task' "$CLAUDE_IMPLEMENTER" "$CODEX_IMPLEMENTER" && grep -Fq 'Use the facts already confirmed by the parent' "$CLAUDE_IMPLEMENTER" "$CODEX_IMPLEMENTER" && grep -Fq 'If a new design decision or broad investigation is required' "$CLAUDE_IMPLEMENTER" "$CODEX_IMPLEMENTER" && grep -Fq 'Outcome: implemented' "$CLAUDE_IMPLEMENTER" "$CODEX_IMPLEMENTER" && grep -Fq '要求に直接必要なproduction code' "$CLAUDE_IMPLEMENTER" "$CODEX_IMPLEMENTER" && ok "implementer: 親の確認済み事実から初回実装だけを行う" || ng "implementerの実装専用境界が不足"
...
grep -Fq '初回実装は専用`implementer` subagent' "$ERRAND_SKILL" && grep -Fq '変更前worktreeがcleanな場合だけ一度再起動' "$ERRAND_SKILL" && grep -Fq '修正をimplementerへ再委任しない' "$ERRAND_SKILL" && grep -Fq '修正する／しない、部分採用、全体拒否の判断は上位モデル' "$ERRAND_SKILL" && ok "errand は専用subagent初回実装・上位モデル判断とfallbackへ固定" || ng "errand の初回実装・修正責務が不正"
...
grep -Fq '[agent_name]が次を直接調査する' "$TDD_SKILL" && grep -Fq '外部workerへ調査を委任しない' "$TDD_SKILL" && ! grep -Fq 'worker/delegate.sh' "$TDD_SKILL" && grep -Fq 'native implementer' "$TDD_SKILL" && grep -Fq '初回実装後の本体コード修正 | 可 | 禁止' "$TDD_SKILL" && ok "tdd は上位モデル調査・下位モデル初回実装・上位修正へ固定" || ng "tdd の調査・実装・修正境界が不正"
