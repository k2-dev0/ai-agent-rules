...
SUBAGENT_RULES="$REPO/skills/SUBAGENT_RULES.md"
...
grep -Fq 'Mermaid `flowchart`' "$COWLICK_FORMAT" && grep -Fq 'Mermaid `stateDiagram-v2`' "$COWLICK_FORMAT" && grep -Fq 'Mermaid `sequenceDiagram`' "$COWLICK_FORMAT" && grep -Fq 'Markdownの決定表' "$COWLICK_FORMAT" && grep -Fq '主要表現を1つ選ぶ' "$COWLICK_FORMAT" && grep -Fq '場合だけ2つ目を加える' "$COWLICK_FORMAT" && ok "cowlickは複雑さに合う主要表現を選択" || ng "cowlickの振る舞い表現の選択契約が不足"
grep -Fq '| 入力・出力 |' "$COWLICK_FORMAT" && grep -Fq '| 判定 |' "$COWLICK_FORMAT" && grep -Fq '| 不変条件 |' "$COWLICK_FORMAT" && grep -Fq '| error |' "$COWLICK_FORMAT" && grep -Fq '| 副作用 |' "$COWLICK_FORMAT" && grep -Fq '| dataの権威 |' "$COWLICK_FORMAT" && grep -Fq '既存symbol・schema field・file path・外部契約名は実名' "$COWLICK_FORMAT" && ok "cowlickの契約表が重要情報を保持" || ng "cowlickの契約表が不足"
grep -Fq '`const`・`let`・`await`' "$COWLICK_FORMAT" && grep -Fq 'local変数名、関数内部のblock構造' "$COWLICK_FORMAT" && grep -Fq '既存patternと参照ruleから一意に決まる' "$COWLICK_FORMAT" && grep -Fq '状態・event・guard・境界値・error・副作用・dataの権威を省略しない' "$COWLICK_FORMAT" && grep -Fq '設計書形式に従い' "$COWLICK_SKILL" && ok "cowlickは実装構文を除き振る舞い契約を保持" || ng "cowlickの省略境界が不正"
...
grep -Fq "../cowlick/DESIGN_FORMAT.md" "$PONYTAIL_CONTRACT" && grep -Fq 'Changesの振る舞い図・契約表を保持' "$PONYTAIL_CONTRACT" && grep -Fq '図の簡略化のために' "$COWLICK_FORMAT" && ok "ponytailは振る舞い契約を失う圧縮を禁止" || ng "ponytailが重要な振る舞い契約を圧縮可能"
...
grep -Fq 'サブエージェントの完了を1回だけ待つ' "$SUBAGENT_RULES" && grep -Fq '中間commentary・status説明・snapshot・再poll' "$SUBAGENT_RULES" && grep -Fq 'timeoutで完了結果がなければ再待機せず' "$SUBAGENT_RULES" && ok "子の起動後は最終結果だけを待つ" || ng "子の待機中に経過出力・再pollが可能"
...
if grep -q '^model = "gpt-5.6-sol"$' .codex/config.toml && grep -q '^model_reasoning_effort = "high"$' .codex/config.toml && grep -q '^default_subagent_model = "gpt-6-astra"$' .codex/config.toml && grep -q '^default_subagent_reasoning_effort = "medium"$' .codex/config.toml; then
  ok "bootstrap codex は親をSol/high、子をAstra/mediumへ固定"
else
...
command grep -rn "hookSpecificOutput" "$REPO/hooks/shell" 2>/dev/null | grep -v hook-io.sh | grep -q . && ng "hook-io 以外にCodex hookスキーマ直書き" || ok "Codex hookスキーマはhook-ioのみ"
