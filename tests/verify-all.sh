...
grep -Fq '../polish/PROCEDURE.md' "$TDD_FROM_DOC" && grep -Fq '実変更pathをまとめて[polish]' "$TDD_FROM_DOC" && grep -Fq 'polish後に[独立レビュー]' "$TDD_FROM_DOC" && grep -Fq '通常起動は検証とcommit後に[独立レビュー]' "$TDD_SKILL" && grep -Fq 'bash [skills_root]/tdd/mark-prompt-done.sh <機能名>' "$TDD_FROM_DOC" && ok "tdd はpolish後の最終差分をreviewしてindexを更新" || ng "tdd のpolish・review・index更新順が不正"
