...
[ -f "$REPO/skills/VERIFICATION_FLOW.md" ] || append_group_failure "検証・修正の共通契約なし"
...
VERIFICATION_FLOW="$REPO/skills/VERIFICATION_FLOW.md"
...
grep -Fq '[skills_root]/INDEPENDENT_REVIEW.md' "$REPO/AGENTS.md" && grep -Fq '../INDEPENDENT_REVIEW.md' "$REPO/skills/tdd/SKILL.md" "$REPO/skills/errand/SKILL.md" && ok "通常実装・tdd・errandの完了前に独立レビューを接続" || ng "独立レビューの入口が不足"
...
grep -Fq "メインが実装する" "$SCENARIO_FLOW" && grep -Fq "メインが指摘範囲を直接修正する" "$VERIFICATION_FLOW" && ! grep -Eq '初回実装を先に代行せず|2回連続|下位モデルに再実装させる' "$SCENARIO_FLOW" "$VERIFICATION_FLOW" && ok "共通フローは初回実装・修正をメインで続行" || ng "直列委任の強制または失敗回数による代行制限が残存"
...
grep -Fq 'SCENARIO_FLOW.md' "$TDD_SKILL" && grep -Fq '../SCENARIO_FLOW.md' "$ERRAND_SKILL" && [ -f "$SCENARIO_FLOW" ] && [ -f "$VERIFICATION_FLOW" ] && grep -Fq '`tdd`と`errand`は、調査後の実装をこの契約へ集約する' "$SCENARIO_FLOW" && grep -Fq 'VERIFICATION_FLOW.md' "$SCENARIO_FLOW" "$POLISH_SKILL" "$UNWIND_SKILL" && ok "tdd・errand・polish・unwindは検証・修正契約を共有" || ng "共通実装・レビューフロー参照が不正"
grep -Fq "共通フローのStep 0〜7" "$TDD_SKILL" && grep -Fq "MODEL_SELECTION.md" "$SCENARIO_FLOW" "$VERIFICATION_FLOW" && ! grep -Eq 'require-implementer|専用定義' "$TDD_SKILL" "$ERRAND_SKILL" && ok "tdd・errandは専用agentなしで直接実装できる" || ng "tdd・errandに専用agentの必須条件が残存"
...
grep -Fq "IMPLEMENTATION_RULES.md" "$VERIFICATION_FLOW" && grep -Fq "制御フローとdata変換を上から追える" "$IMPLEMENTATION_RULES" && grep -Fq "関数ジャンプ" "$IMPLEMENTATION_RULES" && grep -Fq "YAGNI" "$IMPLEMENTATION_RULES" && grep -Fq "filter().map()" "$FUNCTION_RULES" && grep -Fq "reduce()" "$FUNCTION_RULES" && ok "上位モデルは共有基準で保守性と可読性をレビュー" || ng "上位モデルの共有判断基準が不足"
! grep -Eq '自己確認|最終レビュー|再レビュー' "$VERIFICATION_FLOW" "$SCENARIO_FLOW" && grep -Fq 'メインによる全差分の自己レビューは工程に含めない' "$REPO/skills/INDEPENDENT_REVIEW.md" && ok "メインの全差分自己レビューを工程から除外" || ng "自己レビュー工程が残存"
...
grep -Fq "VERIFICATION_FLOW.md#診断のscope帰属" "$SCENARIO_FLOW" && grep -Fq "VERIFICATION_FLOW.md#診断のscope帰属" "$POLISH_SKILL" && grep -Fq "scope-related" "$VERIFICATION_FLOW" && grep -Fq "unrelated" "$VERIFICATION_FLOW" && grep -Fq "uncertain" "$VERIFICATION_FLOW" && grep -Fq "ignored / untracked test" "$VERIFICATION_FLOW" && grep -Fq "どの分類が残っていても完了マークを自動判定せず" "$VERIFICATION_FLOW" && ok "tdd・errand・polishは対象外失敗と完了判断を分離" || ng "対象外失敗がタスク完了を自動阻止"
...
grep -Fq 'ファイルごとには呼ばない' "$TDD_SKILL" && grep -Fq 'formatterがformat差分を自動修正' "$POLISH_SKILL" && grep -Fq '`VERIFICATION_FLOW.md`に従って修正' "$POLISH_SKILL" && grep -Fq '全品質ゲートを再実行' "$POLISH_SKILL" && ok "tdd はpolishを全path一括で原因別に反復" || ng "tdd のpolish実行単位または反復条件が不正"
