...
DENSE_SURVEY_STATUS=$?
if [ -x "$SURVEY_REQUEST_VALIDATOR" ] && bash "$SURVEY_REQUEST_VALIDATOR" "$VALID_SURVEY_REQUEST" | jq -e '.claims[0].id == "C1" and .claims[0].kind == "behavior"' >/dev/null && bash "$SURVEY_REQUEST_VALIDATOR" "$MULTI_SURVEY_REQUEST" | jq -e '.claims | length == 2' >/dev/null && ! bash "$SURVEY_REQUEST_VALIDATOR" "$INVALID_SURVEY_REQUEST" >/dev/null 2>&1 && ! bash "$SURVEY_REQUEST_VALIDATOR" "$TOO_MANY_SURVEY_REQUEST" >/dev/null 2>&1 && [ "$DENSE_SURVEY_STATUS" -ne 0 ] && printf '%s' "$DENSE_SURVEY_ERROR" | jq -e '.errors[] | select(.path == "/claims/0/question" and .message == "C1.question exceeds 2 enumerators")' >/dev/null; then
...
grep -Fq 'native survey subagent' "$ERRAND_SKILL" && ! grep -Fq 'worker/delegate.sh' "$ERRAND_SKILL" && grep -Fq 'native implementer' "$ERRAND_SKILL" && grep -Fq 'テスト、設定、migration、Git、設計資産を変更させない' "$ERRAND_SKILL" && ok "errand はnative調査とnative初回実装を分離" || ng "errand のnative調査・実装境界が不正"
...
grep -Fq '初回実装は専用`implementer` subagent' "$ERRAND_SKILL" && grep -Fq '変更前worktreeがcleanな場合だけ一度再起動' "$ERRAND_SKILL" && grep -Fq '修正をsurveyorまたはimplementerへ再委任しない' "$ERRAND_SKILL" && grep -Fq '修正する／しない、部分採用、全体拒否の判断は上位モデル' "$ERRAND_SKILL" && ok "errand は専用subagent初回実装・上位モデル判断とfallbackへ固定" || ng "errand の初回実装・修正責務が不正"
...
grep -Fq 'source group単位' "$TDD_SKILL" && grep -Fq '最大3体を並行起動' "$TDD_SKILL" && grep -Fq '同じsurveyorへ一度だけ限定follow-up' "$TDD_SKILL" && grep -Fq 'path:line' "$TDD_SKILL" && ok "tdd はsource-localなnative調査と限定follow-upを使う" || ng "tdd のnative調査境界が不正"
grep -Fq '## 1. テストシナリオ候補をまとめて提示する' "$SCENARIO_FLOW" && grep -Fq '採用するシナリオ、外すシナリオ、修正点を指定してください。' "$SCENARIO_FLOW" && grep -Fq '全件採用を既定または要求する言い方をしない' "$SCENARIO_FLOW" && grep -Fq 'どの候補を採用・不採用・修正するかはユーザーが決める' "$SCENARIO_FLOW" && grep -Fq '実装要件を省略する根拠にしてはならない' "$SCENARIO_FLOW" && grep -Fq '選択が確定するまでファイルを変更しない' "$SCENARIO_FLOW" && grep -Fq 'surveyorにはシナリオ、期待値、assertion、fixture構成、テストコードを提案または変更させない' "$SCENARIO_FLOW" && grep -Fq '新しいテストが必要であることだけを理由に停止しない' "$SCENARIO_FLOW" && grep -Fq '`fork_turns: "none"`' "$SCENARIO_FLOW" && grep -Fq '`reasoning_effort: "max"`' "$SCENARIO_FLOW" && grep -Fq 'child agent IDが空' "$SCENARIO_FLOW" && grep -Fq 'wait先が空' "$SCENARIO_FLOW" && ok "共通フローはtest選択権・実装範囲・freshなnative subagentを固定" || ng "共通フローのtest選択権・実装境界またはsubagent起動が不正"
...
grep -Fq 'この出力と完全一致する相対path全件を一括入力' "$POLISH_SKILL" && grep -Fq 'directory、glob、`git diff`で独自に広げたpathを使わない' "$POLISH_SKILL" && grep -Fq 'typecheckとPrisma検証はファイル単位で安全に分割できない' "$POLISH_SKILL" && grep -Fq '実変更pathだけを渡して`unwind`を必ず呼ぶ' "$POLISH_SKILL" && grep -Fq '`unwind`自身では差分を再探索・再検証しない' "$UNWIND_SKILL" && grep -Fq '`list-changed`をもう一度実行しない' "$POLISH_SKILL" && ok "polishとunwindは実変更pathを再探索せず対象化" || ng "polishまたはunwindが実変更pathを再探索"
...
DENY_UNQUOTED_READ=$(jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"IMPL1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .claude/skills/tdd/implementer-read.sh src/rules.ts"}}' | bash "$IMPLEMENTATION_HOOK")
...
DENY_QUOTED_READ_CHAIN=$(jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"IMPL1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .claude/skills/tdd/implementer-read.sh '\''src/rules.ts'\''; git status"}}' | bash "$IMPLEMENTATION_HOOK")
...
IMPLEMENTER_READ_OUTPUT=$(bash .claude/skills/tdd/implementer-read.sh src/rules.ts)
  ...
  ok "implementation-scope: 旧互換hookのsubagent境界を維持"
else
...
jq -n \
  ...
  '{mode:"survey",task_id:"codex-implementation-survey",status:0,output_contract_status:"valid",outcome:"fulfilled",evidence_status:"verified",claim_results:[{id:"C1",status:"fulfilled",evidence_count:1,evidence_status:"verified"}],changed_paths:[],report_file:"report.md",evidence_file:"evidence.md",report_blob:$report_blob,evidence_blob:$evidence_blob}' \
...
CODEX_IMPLEMENTATION_REQUEST=$(jq -cn '{version:4,action:"implement",scope:"codex-implementation",spec:null,implementation_instruction:"対象機能の全要件を初回実装する",worker_tasks:[{task_id:"codex-implementation-survey",result:".codex/tmp/worker/codex-implementation-survey/result.json",report:".codex/tmp/worker/codex-implementation-survey/report.md",evidence:".codex/tmp/worker/codex-implementation-survey/evidence.md",required_claim_ids:["C1"]}],test_scenarios:[{id:"S1",contract:"対象値を期待値へ変更する"}],test_paths:["tests/codex/implementation-flow.test.ts"],red:{command:"npm test",status:1,reason:"期待値差で失敗"},test_exemption:null,allowed_paths:["src/codex-implementation.ts"]}')
...
CODEX_DENY_UNQUOTED_READ=$(jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"CIMPL1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .agents/skills/tdd/implementer-read.sh src/codex-implementation.ts"}}' | bash "$CODEX_IMPLEMENTATION_HOOK")
CODEX_DENY_QUOTED_READ_CHAIN=$(jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"CIMPL1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .agents/skills/tdd/implementer-read.sh '\''src/codex-implementation.ts'\''; git status"}}' | bash "$CODEX_IMPLEMENTATION_HOOK")
...
CODEX_INACTIVE_STATUS=$(bash "$CODEX_CAPTURE_SCOPE" status 2>/dev/null)
if [ "$(echo "$CODEX_INACTIVE_STATUS" | jq -r '.active' 2>/dev/null)" = "false" ]; then ok "codex implementation-scope: inactive statusを取得"; else ng "codex implementation-scope: inactive statusが不正 out=[$CODEX_INACTIVE_STATUS]"; fi
...
if bash "$CODEX_IMPLEMENTATION_VALIDATOR" "$CODEX_IMPLEMENTATION_REQUEST" >/dev/null 2>&1 && \
   jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"CORPHAN1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .agents/skills/polish/capture-scope.sh activate codex-implementation"}}' | bash "$CODEX_IMPLEMENTATION_HOOK" >/dev/null && \
   bash "$CODEX_CAPTURE_SCOPE" activate codex-implementation >/dev/null 2>&1; then
  ok "implementation orphan recovery: fixtureをactivate"
else
  ng "implementation orphan recovery: fixture activate失敗"
fi
CODEX_STATUS_COMMAND="bash .agents/skills/polish/capture-scope.sh status"
CODEX_ALLOW_STATUS=$(jq -n --arg cwd "$PWD" --arg command "$CODEX_STATUS_COMMAND" '{hook_event_name:"PreToolUse",session_id:"CRECOVER1",cwd:$cwd,tool_name:"Bash",tool_input:{command:$command}}' | bash "$CODEX_IMPLEMENTATION_HOOK")
CODEX_DENY_STATUS_CHAIN=$(jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"CRECOVER1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .agents/skills/polish/capture-scope.sh status; git status"}}' | bash "$CODEX_IMPLEMENTATION_HOOK")
CODEX_ACTIVE_STATUS=$(bash "$CODEX_CAPTURE_SCOPE" status 2>/dev/null)
if [ -z "$CODEX_ALLOW_STATUS" ] && \
   [ "$(echo "$CODEX_DENY_STATUS_CHAIN" | jq -r '.hookSpecificOutput.permissionDecision' 2>/dev/null)" = "deny" ] && \
   [ "$(echo "$CODEX_ACTIVE_STATUS" | jq -r '.active' 2>/dev/null)" = "true" ] && \
   [ "$(echo "$CODEX_ACTIVE_STATUS" | jq -r '.feature' 2>/dev/null)" = "codex-implementation" ] && \
   [ "$(echo "$CODEX_ACTIVE_STATUS" | jq -r '.mode' 2>/dev/null)" = "subagent" ] && \
   [ "$(echo "$CODEX_ACTIVE_STATUS" | jq -r '.recoverable' 2>/dev/null)" = "false" ] && \
   [ "$(echo "$CODEX_ACTIVE_STATUS" | jq -r '.dirty_paths | length' 2>/dev/null)" = "0" ]; then
  ok "implementation orphan recovery: active中もstatusだけ安全に取得"
else
  ng "implementation orphan recovery: status契約が不正 status=[$CODEX_ACTIVE_STATUS]"
fi
CODEX_NON_OWNER_END=$(jq -n --arg cwd "$PWD" '{hook_event_name:"SessionEnd",session_id:"COTHER1",cwd:$cwd}')
echo "$CODEX_NON_OWNER_END" | bash $H/session.sh
[ "$(bash "$CODEX_CAPTURE_SCOPE" status 2>/dev/null | jq -r '.mode')" = "subagent" ] && ok "implementation orphan recovery: 非owner終了ではscopeをorphan化しない" || ng "implementation orphan recovery: 非owner終了でscopeが変化"
CODEX_OWNER_END=$(jq -n --arg cwd "$PWD" '{hook_event_name:"SessionEnd",session_id:"CORPHAN1",cwd:$cwd}')
echo "$CODEX_OWNER_END" | bash $H/session.sh
CODEX_ORPHAN_STATUS=$(bash "$CODEX_CAPTURE_SCOPE" status 2>/dev/null)
CODEX_ORPHAN_REPOSITORY=$(echo "$CODEX_ORPHAN_STATUS" | jq -r '.repository' 2>/dev/null)
CODEX_ORPHAN_RECEIPT_ROOT="${TMPDIR:-/tmp}/polish-quality-gate/$(printf '%s' "$CODEX_ORPHAN_REPOSITORY" | cksum | awk '{ print $1 }')"
CODEX_DENY_ORPHAN_WRITE=$(jq -n --arg cwd "$PWD" --arg command $'*** Begin Patch\n*** Update File: src/codex-implementation.ts\n+x\n*** End Patch' '{hook_event_name:"PreToolUse",session_id:"CIMPL2",cwd:$cwd,tool_name:"apply_patch",tool_input:{command:$command}}' | bash "$CODEX_IMPLEMENTATION_HOOK")
if [ "$(echo "$CODEX_ORPHAN_STATUS" | jq -r '.mode' 2>/dev/null)" = "orphaned" ] && \
   [ "$(echo "$CODEX_ORPHAN_STATUS" | jq -r '.recoverable' 2>/dev/null)" = "true" ] && \
   [ -f "$CODEX_ORPHAN_RECEIPT_ROOT/implementation.active/request.json" ] && \
   [ "$(echo "$CODEX_DENY_ORPHAN_WRITE" | jq -r '.hookSpecificOutput.permissionDecision' 2>/dev/null)" = "deny" ]; then
  ok "implementation orphan recovery: owner終了時にrequestを保持して書き込み停止"
else
  ng "implementation orphan recovery: orphan化契約が不正 status=[$CODEX_ORPHAN_STATUS]"
fi
...
if bash "$CODEX_IMPLEMENTATION_VALIDATOR" "$CODEX_IMPLEMENTATION_REQUEST" >/dev/null 2>&1 && \
   jq -n --arg cwd "$PWD" '{hook_event_name:"PreToolUse",session_id:"CLEGACY1",cwd:$cwd,tool_name:"Bash",tool_input:{command:"bash .agents/skills/polish/capture-scope.sh activate codex-implementation"}}' | bash "$CODEX_IMPLEMENTATION_HOOK" >/dev/null && \
   bash "$CODEX_CAPTURE_SCOPE" activate codex-implementation >/dev/null 2>&1; then
  CODEX_RECEIPT_ROOT="${TMPDIR:-/tmp}/polish-quality-gate/$(git rev-parse --show-toplevel | cksum | awk '{ print $1 }')"
