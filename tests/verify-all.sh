...
grep -Fq "メインが実装する" "$SCENARIO_FLOW" && grep -Fq "メインが指摘範囲を直接修正する" "$FIX_FLOW" && ! grep -Eq '初回実装を先に代行せず|2回連続|下位モデルに再実装させる' "$SCENARIO_FLOW" "$FIX_FLOW" && ok "共通フローは初回実装・修正をメインで続行" || ng "直列委任の強制または失敗回数による代行制限が残存"
