...
grep -q 'MAX_EVIDENCE_REFERENCES="20"' "$WORKER_RUNNER" && grep -q 'RECOMMENDED_EVIDENCE_REFERENCES="12"' "$WORKER_RUNNER" && grep -q 'MAX_SURVEY_CLAIMS="6"' "$WORKER_RUNNER" && grep -q 'MAX_SURVEY_EVIDENCE_PER_CLAIM="3"' "$WORKER_RUNNER" && grep -Fq '各claimは1〜3範囲、全claimの合計は最大20範囲、推奨12範囲以下' "$WORKER_RUNNER" && ok "worker evidence: surveyのclaim・範囲数を圧縮" || ng "worker evidenceのclaim・範囲数上限が不足"
