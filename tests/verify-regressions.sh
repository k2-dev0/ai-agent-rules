...
for agent in claude codex; do
  ...
  check grep -Fq '成功時は`{"score":<1〜10の整数>,"reason":"behavior=<0〜2>, scope=<0〜2>, state=<0〜2>, impact=<0〜2>, verification=<0〜2>; <主な根拠>"}`' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
  ...
  check grep -Fq '`score`は理由の各軸値に`max(1, 合計)`を適用した値と一致' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
  check grep -Fq '最高点の軸とコード上の根拠を優先して300文字以内' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
  check grep -Fq '0点は対象要素がない場合だけでなく、調査により定型で追加判断が不要と確認できた場合も含む' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
  check grep -Fq '一つでも高い整合性を要する単位があれば、定型部分との平均で薄めない' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
  check grep -Fq 'review findingのseverity・件数' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
  check grep -Fq '検証から保存までの競合' "$target/$skill_root/DIFFICULTY_CONTRACT.md"
...
for agent in claude codex; do
  ...
  check test "$(jq '[.hooks.PreToolUse[] | .matcher as $matcher | select("Bash" | test($matcher)) | .hooks[].command | select(contains("protect-git.sh"))] | length' "$settings")" = 1
  printf 'source\n' > source.txt
  printf 'original\n' > existing.txt
