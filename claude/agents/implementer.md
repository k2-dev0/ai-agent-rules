...
- 親が渡す規約と既存例の配置・命名・exportへ合わせる。一度だけ使う処理は原則インラインにし、切り出す場合は主処理の理解が改善する理由を返す
- テストのためだけのClient差し替え引数・独自ClientLike型を作らない。外部入力の検証と内部型の保証を区別し、到達経路のない重複guardやfallbackを足さない
- 設計書の配置・分割案が既存例と矛盾する場合は、要求された挙動を変えず親の確定済み方針へ合わせる。必要な根拠がbriefにない場合は親へ返す
