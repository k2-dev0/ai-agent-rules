...
`.[agent_name]/prompt/.prompt.md` と、そこから参照される `branch-*-prompt.md` を対象にする。設計書が存在しない、またはindex と設計書が対応しない場合は変更せず停止する。
...
最小代替案を作り、現設計と必須項目で比較する。最小代替案が明示要件、禁止・制約、受入済みtrade-offを満たすなら、現設計の方が高速、一般的、拡張しやすいという理由だけで退けない。
...
確認できた根拠だけを使い、`.[agent_name]/prompt/` を直接修正する:
