...
2. **横断 topology**: 全設計書の入口、共有責務、caller・consumer、既存境界と新設予定の public endpoint、queue、scheduler、worker、serverless function、外部接続、永続化、global/shared変更を一列にする。bug修正では症状とroot causeも対応付ける
3. **要素対応表**: 新設要素と対象ファイルを、明示要件または既存制約へ対応付ける。ファイル単位で止めず、追加予定のexport、関数、定数、型、classごとに直接の外部consumerを列挙する。同一ファイル内の呼び出しは別consumerに数えず、内部構成のためだけに分割した要素は統合候補、別の値から導ける定数は削除候補にする。設計選択にしか対応しないものも削除候補にする
...
6. **反例表**: 数値計算、境界比較、sort、prefix選択、候補選択、組み合わせ処理がある場合、正常例だけでなく、等号境界、値の混在、同値、入力順の変更、より少ない候補で同等以上になる場合から該当ケースを具体値で作り、期待結果と根拠を示す。具体例で破れる規則を確定要件として残さない
...
6. 同じ修正を複数callerへ置く代わりに、全経路が通るroot causeの共有責務へ一度だけ置けないか
7. export、関数、定数、型、classの直接の外部consumerはどのファイルか。同一ファイル内の呼び出しを水増しせず、repositoryの抽象化規約に照らして統合またはローカル化できないか
8. 数値、順序、選択規則は反例表の全ケースを満たすか。満たさない場合、その規則を要件として固定せず、入力契約の不足またはユーザー判断として扱えないか
...
- 入口から全caller・consumer、共有責務、副作用まで実経路を追い、bug修正では症状とroot causeを分けた
- 新しいruntime boundaryとglobal/shared変更を加えない最小代替案を比較した
- 新設要素と対象ファイルが明示要件または既存制約へ直接対応している
- 追加予定のexport、関数、定数、型、classごとに直接の外部consumerを確認し、同一ファイル内の呼び出しをconsumerとして水増ししていない
- 数値、順序、選択ロジックを具体値の反例表で検証し、破れる設計選択を確定要件として残していない
- Changesが実装時の再設計を要求せず、重要な分岐・式・順序・入出力契約を構造化疑似コードで保持している
...
- 意図的な簡素化に既知の上限がある場合、測定可能な再検討条件を設計へ残した
- 非自明な処理を守る最小の実行可能なテストを設計から落としていない
- 未決定事項がない
