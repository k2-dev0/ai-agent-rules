...
3. [agent_name]が各設計書をコードベースと照合する
   - サブエージェントへ調査を委任しない
4. 設計書形式に従い、`.[agent_name]/prompt/`の`.prompt.md`と`branch-<機能名>-prompt.md`を直接更新する
   - 初回はWrite、改訂はEdit
5. 設計書形式の必須sectionを埋め、設計書形式の実装情報を保持する
   - 設計全体の監査は後段のponytailへ渡す
