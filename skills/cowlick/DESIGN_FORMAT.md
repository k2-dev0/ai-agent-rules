...
cowlick はこの文書を全文読み、次の形式を崩さない。

## ファイル構成

`.[agent_name]/prompt/`には次だけを置く。
...
## 設計根拠
- <明示要件・禁止制約・受入済みtrade-off・既存制約と、変更可能な設計選択を区別する>
- <新設要素のconsumer、同じ責務の既存例path:lineとexport、適用する規約。インライン・既存処理の再利用では読みにくい／満たせない理由>
...
疑似コード上の配列・オブジェクトは、引数・変数名・参照式でも末尾に形を付けて明示する。配列は`[]`、オブジェクトは`{}`とし、例えば配列なら`候補一覧[]`、オブジェクトなら`候補一覧{}`と書く。型を示す記号は参照時も省略せず、`候補一覧[].length`、`候補一覧[].slice(...)`、`利用結果{}`のように書く。
...
export async function 利用内容を確定する関数(使用方法, 入力件数, 候補一覧[], 上限件数) {
  ...
  const 利用結果{} = { 成功: true, 使用件数, 使用候補一覧: 使用候補一覧[] }
  return 利用結果{}
}
```

対象ファイルと参照ruleは正確な相対パスで列挙する。[設計・実装の判断基準](../IMPLEMENTATION_RULES.md)に沿って確認した根拠を「設計根拠」に記録する。

完了条件にはChangesの実装、必要な検証、構造・配置・命名のレビューを含める。`tdd`は1枚だけ読むため、別設計書を読まないと判定できない条件を置かない。
