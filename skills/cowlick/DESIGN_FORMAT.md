...
疑似コード上の配列・オブジェクトは、引数・変数名の末尾に形を付けて明示する。配列は`[]`、オブジェクトは`{}`とし、例えば配列なら`候補一覧[]`、オブジェクトなら`候補一覧{}`と書く。これは型を示す記号なので、値を式で参照するときは`候補一覧.length`のように名前から記号を外す。
...
export async function 利用内容を確定する関数(使用方法, 入力件数, 候補一覧[], 上限件数) {
  ...
  const 利用結果{} = { 成功: true, 使用件数, 使用候補一覧 }
  return 利用結果
}
```
