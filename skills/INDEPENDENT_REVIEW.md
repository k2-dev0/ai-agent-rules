...
| 環境・条件 | role |
|---|---|
| Codex：通常 | `code-reviewer`（Astra / medium） |
| Codex：同じタスクの実装レビューでcritical検出後 | `code-reviewer-critical`（Astra / medium） |
...
Codexのレビューが対象の一致した`reviewed`結果で`critical`を返したら、修正・採否判断の前に同じ対象を`code-reviewer-critical`で一度だけ新規レビューする。過去の指摘は入力へ含めず、以後このタスクの修正後レビューも同roleを使う。モデル・effortはAstra / mediumを維持し、criticalの再検出だけでは同一入力を再実行しない。
