...
| 環境・条件 | role |
|---|---|
| Codex：通常 | `code-reviewer`（Astra / high） |
| Codex：同じタスクの実装レビューでcritical検出後 | `code-reviewer-critical`（Astra / xhigh） |
...
Codexのhighレビューが対象の一致した`reviewed`結果で`critical`を返したら、修正・採否判断の前に同じ対象を`code-reviewer-critical`で一度だけ新規レビューする。過去の指摘は入力へ含めず、以後このタスクの修正後レビューも同roleを使う。xhighの結果で以下を処理し、criticalの再検出だけでは同一入力を再実行しない。
...
critical検出によるhighからxhighへの初回昇格を除き、同一session・HEAD・要求・制約で確認済みの結果は再利用し、同じ入力で繰り返し起動しない。却下した指摘は根拠を短く残す。要件・制約が変わった場合は再レビューする。
