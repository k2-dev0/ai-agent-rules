...
メインの自己確認、必要なtest・静的検査・整形・polish、commit後、完了報告前に一度実行する。途中の自己確認では子を起動しない。ネスト検出だけで代用しない。
...
- 指摘あり：メインが根拠を確認し、採用・却下を判断する。採用分は[モデル選択](MODEL_SELECTION.md)に従って直接修正し、影響する検証・整形・必要なpolish・commit後、同じ`review_base`と新HEADを新規の子へ渡す。
