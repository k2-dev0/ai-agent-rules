...
候補に対する修正主体は[上位モデルのレビューと下位モデルの再実装](../tdd/REVIEW_FLOW.md)を全文読み、大小判定する。読み取り専用workerはこの判定と再実装を担当しない。
...
4. 振る舞いを保てる案があれば、`REVIEW_FLOW.md`で修正を大小判定する。小さい問題だけ上位モデルが検出済み箇所へ直接適用する。大きい問題は修正案、保つべき振る舞い、変更禁止範囲、対象test commandを確定し、実装用の下位モデルをeffort `max`で起動して再実装させる。上位モデルが実差分を再レビューした後、対象テスト・型検査・lintと、親の`polish`が実行した同じpackageのbuildを再実行する。親がbuildを`not run`とした場合はその理由を引き継ぎ、新しいbuild commandを発明しない。コードを変更してコミットした場合は新しいHEADからtask-idを作り直し、Step 1から再検出する。
