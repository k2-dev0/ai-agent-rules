...
- 起動を一度だけ簡潔に通知し、tool上限の1時間でサブエージェントの完了を1回だけ待つ
  - 完了まで中間commentary・status説明・snapshot・再poll・催促・待機だけのモデル交代を行わない
  - timeoutで完了結果がなければ再待機せず、未完了として呼出元へ返す
  - 完了を受けたら結果だけを処理して元の作業を続ける
