...
- 候補が未生成、timeout、最終応答欠落なら新しいtask-idと`--retry-of`で1回だけ再委任する。2回続けて応答に失敗した場合、または明示的な認証失敗があった場合だけ上位モデルが初回実装を引き継ぐ
...
2. workerの`survey`を必ず1回実行し、共通フローのO1〜O5、最寄りの同型実装1件、置換する要素、対象pathを返させる。網羅監査は依頼しない。
