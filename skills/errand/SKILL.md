...
依頼にある識別子、path、番号、固有名詞を省略・翻訳・一般化しない。これらを探索起点にし、[agent_name]が次を直接確認する。下位モデル、surveyor subagent、外部workerへ調査を委任しない。
...
要求判断を直接支える最小範囲から読み、確認済み事実、根拠の`path:line`、想定変更先、test・検証command、未確認事項を実装briefに保持する。必須情報が不足する場合は[agent_name]が追加調査し、一意性を確定できなければ変更せず停止する。

## 実行手順

1. 依頼から公開挙動、完了条件、既知の識別子・機能語を固定する。
2. 共通フローのStep 0でimplementer定義をpreflightし、[agent_name]が必要なリポジトリ事実を直接調査する。
...
6. ユーザー依頼の全要件、[agent_name]が確認した事実、test_scenarios、Red、想定変更先、変更禁止カテゴリを一つの自然文briefにする。test_scenariosの採否で実装指示を削らない。想定変更先は認可リストにしない。同型実装から名前・内容を一意に決められる新規本体ファイルも要件内なら許可する。
