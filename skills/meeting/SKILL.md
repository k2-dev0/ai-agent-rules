...
コードベースの探索、根拠収集、設計判断、横断比較、設計書の採否、ユーザーへ提示する選択肢は、オーケストレーターである[agent_name]が担当する。調査を下位モデル、subagent、外部workerへ委任しない。下位モデルは後段の初回実装、上位モデルが確定した大きいレビュー指摘の再実装、確定済みpathに対するネスト候補抽出などの機械的QAだけに使う。
...
- 各内部skillで上位モデルが直接調査しても必須根拠を得られない場合は、失敗した範囲を示し、推測で先へ進まない
