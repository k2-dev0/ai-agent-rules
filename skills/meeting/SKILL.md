...
Codexのrepository調査はDeepSeek、設計は親Astra / xhighが担当する。Claudeのpreflight・cowlickの調査はメインが行う。ponytailの監査は履歴を継承しない専用子へ渡す。内部工程の選択・再実行は自分で行い、ユーザーにしか決められない事項を一件ずつ質問する。回答済みの事項は聞き直さない。
