...
コードベースの探索、根拠収集、設計判断、横断比較、設計書の採否、ユーザーへ提示する選択肢は、オーケストレーターである[agent_name]が担当する。調査を下位モデル、subagent、外部workerへ委任しない。下位モデルは後段の初回実装と、確定済みpathに対するネスト候補抽出などの機械的QAだけに使う。
