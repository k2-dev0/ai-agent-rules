...
`$meeting` の明示呼び出しでだけ起動する。ユーザーが自然言語で要件、軽微な修正、単一ファイルの追加・変更、質問、調査、レビューを依頼しただけでは起動してはならない。その依頼は、通常の対応フローで直接扱う。

新規機能・複数ファイルにまたがる変更・設計判断を含む依頼でも、ユーザーが `$meeting` を呼んでいなければ、このスキルを勝手に挟まない。設計 workflow が必要だと判断した場合だけ、通常の会話で `$meeting` を提案できる。ただし提案は起動許可ではない。
...
`ponytail_ready` の文字列だけでは通過させない。全設計書の横断topology、入口からcaller・consumer・共有責務・副作用までの実経路、bug修正における症状とroot causeの分離、最小代替案との比較、新設要素と要件の対応、原因・緩和対の削除確認、残した主要要素の根拠が揃うことを確認する。追加予定のexport、関数、定数、型、classは直接の外部consumerが示され、同一ファイル内の呼び出しがconsumerとして水増しされていないことも確認する。数値、順序、選択ロジックがあれば、等号境界、値の混在、同値、入力順の変更など該当する具体値の反例表を確認する。意図的な簡素化に既知の上限があれば測定可能な再検討条件があり、非自明な処理を守る最小の実行可能なテストが設計に残り、未決定事項がなく、ponytail revision が現在の draft revision と一致するまで最終承認へ進まない。
