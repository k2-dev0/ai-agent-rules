...
`ponytail_ready` の文字列だけでは通過させない。全設計書の横断topology、入口からcaller・consumer・共有責務・副作用までの実経路、bug修正における症状とroot causeの分離、最小代替案との比較、新設要素と要件の対応、原因・緩和対の削除確認、残した主要要素の根拠が揃うことを確認する。意図的な簡素化に既知の上限があれば測定可能な再検討条件があり、非自明な処理を守る最小の実行可能なテストが設計に残り、未決定事項がなく、ponytail revision が現在の draft revision と一致するまで最終承認へ進まない。
